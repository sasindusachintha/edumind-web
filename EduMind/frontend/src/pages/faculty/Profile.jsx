...
export default function FacultyProfile() {
  ...
  return (
    <div>
      <PageHeader title="Profile" subtitle="Your faculty account details." />

      <div className="mb-5 flex items-center gap-3">
        <IdTag name={profile.name} idNumber={profile.faculty_no} sub={profile.designation} accent={ROLE_THEME.faculty.accent} />
      </div>

      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        <div className="card">
          <div className="mb-4 flex items-center gap-2"><UserCircle size={16} className="text-faculty" /><p className="font-display text-sm font-semibold text-ink">Profile details</p></div>
          <form onSubmit={handleProfileSubmit} className="space-y-3.5">
            ...
            <button type="submit" className="btn-primary-faculty" disabled={saving}>{saving ? 'Saving…' : 'Save changes'}</button>
          </form>
        </div>

        <div className="card">
          <div className="mb-4 flex items-center gap-2"><KeyRound size={16} className="text-faculty" /><p className="font-display text-sm font-semibold text-ink">Change password</p></div>
          <form onSubmit={handlePasswordSubmit} className="space-y-3.5">
            ...
            <button type="submit" className="btn-primary-faculty" disabled={pwSaving}>{pwSaving ? 'Updating…' : 'Update password'}</button>
          </form>
        </div>
      </div>
    </div>
