...
export default function FacultyNotes() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    facultyApi.listNotes().then((res) => setNotes(res.data)).finally(() => setLoading(false));
  }, []);
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editing) {
        await facultyApi.updateNote(editing.id, form);
        push('Note updated.');
      } else {
        await facultyApi.createNote(form);
        push('Note created.');
      }
      setModalOpen(false);
      load();
  ...
  async function handleDelete() {
    setDeleting(true);
    try {
      await facultyApi.deleteNote(deleteTarget.id);
      push('Note deleted.');
  ...
  return (
    <div>
      <PageHeader
        title="Notes"
        subtitle="Personal notes — share with colleagues if useful."
        actions={<button className="btn-primary-faculty" onClick={openCreate}><Plus size={16} /> New note</button>}
      />

      {loading ? (
        <div className="flex justify-center py-16"><Spinner size={24} /></div>
      ) : notes.length === 0 ? (
        <EmptyState icon={StickyNote} title="No notes yet" message="Jot down reminders, observations, or anything worth keeping track of." />
      ) : (
          ...
          {notes.map((n) => (
            <div key={n.id} className="card">
              <div className="flex items-start justify-between gap-2">
                ...
                {!!n.is_shared && <Share2 size={14} className="mt-0.5 flex-shrink-0 text-faculty" />}
              </div>
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  <button className="rounded-md p-1.5 text-ink/45 hover:bg-paper hover:text-faculty" onClick={() => openEdit(n)} aria-label="Edit"><Pencil size={13} /></button>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit note' : 'New note'}>
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button type="submit" className="btn-primary-faculty" disabled={saving}>{saving ? 'Saving…' : 'Save'}</button>
          </div>
        </form>
      </Modal>
